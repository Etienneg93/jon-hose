...
> - **#4 fire readability — SHIPPED** (main b2c9926).
> - **#11 §1 input buffer + neutral dash — SHIPPED** (main 958a499; body collision removed too). §2+ (settings panel, remap, colorblind) open.
> - **#3 juice items 1–5 — SHIPPED** (main ba5ba28). Items 6–8 open. Design rulings recorded in the plan doc: no combat freezes ever; wetness is the enemy hurt read.
> - **#7 §1 persistence — BUILT BUT PARKED by design ruling:** fresh run every boot is intentional until the game is longer and the elemental altar cutscenes exist. Code parked on `church-persistence` branch. §2+ (death tithe, nave fills) parked with it.
> - Everything else open. Next quick win: **#2 Part 1 curve fixes**.
