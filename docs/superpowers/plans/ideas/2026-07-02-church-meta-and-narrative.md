...
> **STATUS: PARKED (2026-07-02 design ruling).** Persistence (§1) is built and tested on the *church-persistence* branch but deliberately NOT merged: every boot must be a fresh run — no yesterday-buffs on a new run — until the game is expanded and the elemental altar cutscenes are done. Revisit this whole spec then.
