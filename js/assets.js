...
(function () {
  ...
  const AM_H = 58;
  // Per-pose draw heights: masters have different body postures, so a
  ...
  Assets.register("assman", (p, opt, ctx, x, y, facing) => {
    ...
    const drawH = AM_POSE_H[key] || AM_H;
    const scale = drawH / img.naturalHeight;
    const dw = Math.round(img.naturalWidth * scale);
    ctx.save();
    ctx.translate(x, y);
  ...
  // Ass Man talks in two wardrobes: costumed for the defeat outro, casual
  // (hoodie + shades) for any beat before the reveal. Same mouth-pair idiom.
  {
  ...
  {
    const _closed = JH.Loader.img("sprites/assman/assman-portrait-casual-mouthclosed.png");
